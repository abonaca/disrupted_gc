...
- orbits similar to surviving clusters, disruption just a function of mass
-- picture that many came on the same orbit (eg w ge, sgr, etc)
-- the attrition is a function of orbit: more disruption at small radii expected, also where we expect more streams (hard to tell due to incompleteness)
-- disruption as function of ecc -- not enhanced at low ecce -- maybe isn't observed as a stream
...
\caption{
\label{fig:mhost}
Mass fraction of the natal galaxy constituted by the progenitors of low-metallicity fossil streams (blue) and globular clusters (black) as a function of the natal galaxy mass.
...
On average, fossil stream progenitors constituted a smaller fraction of their natal galaxy's mass than globular clusters, but the progenitors of the most metal-poor fossil streams likely represented a significant fraction of the lowest-mass galaxies. % ever to exist.
}
\end{figure}
...
% The sample is restricted to fossil streams with known metallicities, which are converted to a natal galaxy mass using the galaxy mass-metallicity relation between $z=3$ and $z=6$.
shown in figure~\ref{fig:mhost}: blue and black gc with metallicities (cite sources of metallicity)
Error bars combine metallicity uncertainties with the evolution of the mass-metallicity relation across this redshift range.
...
% - from charlie
% - radial distribution of streams: more closer to the center?

% - mass function slope
% -- expected -1, means low-mass are missing
% -- we recover a flat slope:
...
% - uncovering the disrupted population
% -- what fraction do these streams constitute (large enough that plausible a complete stream census would recover all?)
% 1) no bulge
% 2) only see streams that disrupted recently
% 3) bulk of the cluster disruption by tidal shocks and not evaporation (kruijssen2015b)

% * implications for individual streams
% % - lower mass -> more puffy? check in surviving clusters relation between mass and size
% % -- disruption rate independent of cluster radius
% - lower mass -> implications for epicycles: check andreas' paper
% - make specific comments for individual streams (e.g., gd-1)
...
% - hyades tails discovered -> is there a mass gap -- probably not, but there should be an age gap, need massive clusters to survive for a hubble time, but not to survive for a Myr, upper mass limit decreases -> more streams to be discovered?
% - the population we see is in the open cluster regime, but only now, probably not before tidal shocks, survived bc kicked out in the halo at high z, in the disk would have been disrupted

% * population-level inference
% - low mass, but if mdf the same as host, those at low metallicity are a high fraction of the host galaxy mass
% --> window into the lowest mass, most dm dominated galaxies -> spatially non-uniform? (sims) fesc for reionization?
% -> higher for clumpier medium
% - for reionization most relevant clumpiness in the gas
% - don't expect bound clusters to be dominant mode of sf -- sylgr might just be stochasticity -- sylgr helped keep the galaxy ionized, dominated at birth, integrated combined less
% - what were they in context of their host galaxies
% - need metallicity
% - don't dominate the galaxy, sometimes they do


% - low mass, recently disrupted, would expect to detect w jwst?
% --> more numerous gc in the past, relation to clumpy disks at high z? -- more metal rich
